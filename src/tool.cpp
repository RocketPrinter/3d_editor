...
const Tool triangle_tools[] = {
    ...
    Tool {
        .name = "Eyedropper",
        .click = [](World& world) {
            // averages the color between all selected triangles and sets the paint color
            int count = 0;
            float r=0,g=0,b=0;
            for (auto kvp: world.selection) {
                for (int i: kvp.second) {
                    auto col = kvp.first->triangle_colors[i];
                    r += col.r, g += col.g, b += col.b, count++;
                }
            }
            world.paint_color = {
                    .r = (unsigned char)(r / count),
                    .g = (unsigned char)(g / count),
                    .b = (unsigned char)(b / count),
                    .a = 255 };
        }
    },
...
Tool {
        ...
        .click = [](World& world) {
            for (auto kvp: world.selection) {
                for (int i: kvp.second) {
                    kvp.first->triangle_colors[i] = world.paint_color;
                }
            }
        }
    }
...
void show_tools(World& world) {
    ...
    if (world.selection_mode == SelectionMode::Vertex) {
        ...
        ray::DrawRectangle(x_offset, screenHeight - (bottom_padding + rect_height), rect_height, rect_height, world.paint_color);
    }
}
