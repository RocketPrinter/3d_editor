...
*       layouts must be defined manually on controls drawing, providing the right bounds ray::Rectangle for it.
*
...
*                         REVIEWED: Local ray::GetCodepointNext(), aligned with raylib
...
*
*           - ray::Vector2 ray::GetMousePosition(void);
*           - float ray::GetMouseWheelMove(void);
*           - bool IsMouseButtonDown(int button);
*           - bool IsMouseButtonPressed(int button);
*           - bool IsMouseButtonReleased(int button);
*           - bool IsKeyDown(int key);
*           - bool IsKeyPressed(int key);
*           - int ray::GetCharPressed(void);         // -- GuiTextBox(), GuiValueBox()
*
*           - void DrawRectangle(int x, int y, int width, int height, ray::Color color); // -- GuiDrawRectangle()
*           - void DrawRectangleGradientEx(ray::Rectangle rec, ray::Color col1, ray::Color col2, ray::Color col3, ray::Color col4); // -- GuiColorPicker()
*
*           - ray::Font ray::GetFontDefault(void);                            // -- GuiLoadStyleDefault()
*           - ray::Font ray::LoadFontEx(const char *fileName, int fontSize, int *codepoints, int codepointCount); // -- GuiLoadStyle()
*           - Texture2D LoadTextureFromImage(ray::Image image);          // -- GuiLoadStyle(), required to load texture from embedded font atlas image
*           - void SetShapesTexture(Texture2D tex, ray::Rectangle rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)
*           - char *ray::LoadFileText(const char *fileName);             // -- GuiLoadStyle(), required to load charset data
*           - void ray::UnloadFileText(char *text);                      // -- GuiLoadStyle(), required to unload charset data
*           - const char *ray::GetDirectoryPath(const char *filePath);   // -- GuiLoadStyle(), required to find charset/font file from text .rgs
*           - int *ray::LoadCodepoints(const char *text, int *count);    // -- GuiLoadStyle(), required to load required font codepoints list
*           - void ray::UnloadCodepoints(int *codepoints);               // -- GuiLoadStyle(), required to unload codepoints list
*           - unsigned char *ray::DecompressData(const unsigned char *compData, int compDataSize, int *dataSize); // -- GuiLoadStyle()
*
*   CONTRIBUTORS:
...
//----------------------------------------------------------------------------------
    ...
    // ray::Vector2 type
    typedef struct ray::Vector2 {
        float x;
        float y;
    } ray::Vector2;

    // ray::Vector3 type                 // -- ConvertHSVtoRGB(), ConvertRGBtoHSV()
    typedef struct ray::Vector3 {
        float x;
        float y;
        float z;
    } ray::Vector3;

    // ray::Color type, RGBA (32bit)
    typedef struct ray::Color {
        unsigned char r;
        unsigned char g;
        unsigned char b;
        unsigned char a;
    ...
    // TODO: Texture2D type is very coupled to raylib, required by ray::Font type
    ...
    // ray::Image, pixel data stored in CPU memory (RAM)
    typedef struct ray::Image {
        void *data;             // ray::Image raw data
        int width;              // ray::Image base width
        int height;             // ray::Image base height
        int mipmaps;            // Mipmap levels, 1 by default
        int format;             // Data format (PixelFormat type)
    } ray::Image;

    // ray::GlyphInfo, font characters glyphs info
    typedef struct ray::GlyphInfo {
        int value;              // Character value (Unicode)
        int offsetX;            // Character offset X when drawing
        int offsetY;            // Character offset Y when drawing
        int advanceX;           // Character advance position X
        ray::Image image;            // Character image data
    } ray::GlyphInfo;

    // TODO: ray::Font type is very coupled to raylib, mostly required by GuiLoadStyle()
    // It should be redesigned to be provided by user
    typedef struct ray::Font {
        int baseSize;           // Base size (default chars height)
        int glyphCount;         // Number of glyph characters
        int glyphPadding;       // Padding around the glyph characters
        Texture2D texture;      // Texture atlas containing the glyphs
        ray::Rectangle *recs;        // Rectangles in texture for the glyphs
        ray::GlyphInfo *glyphs;      // Glyphs info data
    } ray::Font;
#endif
...
// ray::Font set/get functions
RAYGUIAPI void GuiSetFont(ray::Font font);                           // Set gui custom font (global state)
RAYGUIAPI ray::Font GuiGetFont(void);                                // Get gui custom font (global state)
...
RAYGUIAPI void GuiDrawIcon(int iconId, int posX, int posY, int pixelSize, ray::Color color); // Draw icon using pixel size at specified position
#endif
...
RAYGUIAPI int GuiWindowBox(ray::Rectangle bounds, const char *title);                                       // Window Box control, shows a window that can be closed
RAYGUIAPI int GuiGroupBox(ray::Rectangle bounds, const char *text);                                         // Group Box control with text name
RAYGUIAPI int GuiLine(ray::Rectangle bounds, const char *text);                                             // Line separator control, could contain text
RAYGUIAPI int GuiPanel(ray::Rectangle bounds, const char *text);                                            // Panel control, useful to group controls
RAYGUIAPI int GuiTabBar(ray::Rectangle bounds, const char **text, int count, int *active);                  // Tab Bar control, returns TAB to be closed or -1
RAYGUIAPI int GuiScrollPanel(ray::Rectangle bounds, const char *text, ray::Rectangle content, ray::Vector2 *scroll, ray::Rectangle *view); // Scroll Panel control

// Basic controls set
RAYGUIAPI int GuiLabel(ray::Rectangle bounds, const char *text);                                            // Label control, shows text
RAYGUIAPI int GuiButton(ray::Rectangle bounds, const char *text);                                           // Button control, returns true when clicked
RAYGUIAPI int GuiLabelButton(ray::Rectangle bounds, const char *text);                                      // Label button control, show true when clicked
RAYGUIAPI int GuiToggle(ray::Rectangle bounds, const char *text, bool *active);                             // Toggle Button control, returns true when active
RAYGUIAPI int GuiToggleGroup(ray::Rectangle bounds, const char *text, int *active);                         // Toggle Group control, returns active toggle index
RAYGUIAPI int GuiToggleSlider(ray::Rectangle bounds, const char *text, int *active);                        // Toggle Slider control, returns true when clicked
RAYGUIAPI int GuiCheckBox(ray::Rectangle bounds, const char *text, bool *checked);                          // Check Box control, returns true when active
RAYGUIAPI int GuiComboBox(ray::Rectangle bounds, const char *text, int *active);                            // Combo Box control, returns selected item index

RAYGUIAPI int GuiDropdownBox(ray::Rectangle bounds, const char *text, int *active, bool editMode);          // Dropdown Box control, returns selected item
RAYGUIAPI int GuiSpinner(ray::Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode); // Spinner control, returns selected value
RAYGUIAPI int GuiValueBox(ray::Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode); // Value Box control, updates input text with numbers
RAYGUIAPI int GuiTextBox(ray::Rectangle bounds, char *text, int textSize, bool editMode);                   // Text Box control, updates input text

RAYGUIAPI int GuiSlider(ray::Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue); // Slider control, returns selected value
RAYGUIAPI int GuiSliderBar(ray::Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue); // Slider Bar control, returns selected value
RAYGUIAPI int GuiProgressBar(ray::Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue); // Progress Bar control, shows current progress value
RAYGUIAPI int GuiStatusBar(ray::Rectangle bounds, const char *text);                                        // Status Bar control, shows info text
RAYGUIAPI int GuiDummyRec(ray::Rectangle bounds, const char *text);                                         // Dummy control for placeholders
RAYGUIAPI int GuiGrid(ray::Rectangle bounds, const char *text, float spacing, int subdivs, ray::Vector2 *mouseCell); // Grid control, returns mouse cell position

// Advance controls set
RAYGUIAPI int GuiListView(ray::Rectangle bounds, const char *text, int *scrollIndex, int *active);          // List View control, returns selected list item index
RAYGUIAPI int GuiListViewEx(ray::Rectangle bounds, const char **text, int count, int *scrollIndex, int *active, int *focus); // List View with extended parameters
RAYGUIAPI int GuiMessageBox(ray::Rectangle bounds, const char *title, const char *message, const char *buttons); // Message Box control, displays a message
RAYGUIAPI int GuiTextInputBox(ray::Rectangle bounds, const char *title, const char *message, const char *buttons, char *text, int textMaxSize, bool *secretViewActive); // Text Input Box control, ask for text, supports secret
RAYGUIAPI int GuiColorPicker(ray::Rectangle bounds, const char *text, ray::Color *color);                        // ray::Color Picker control (multiple color controls)
RAYGUIAPI int GuiColorPanel(ray::Rectangle bounds, const char *text, ray::Color *color);                         // ray::Color Panel control
RAYGUIAPI int GuiColorBarAlpha(ray::Rectangle bounds, const char *text, float *alpha);                      // ray::Color Bar Alpha control
RAYGUIAPI int GuiColorBarHue(ray::Rectangle bounds, const char *text, float *value);                        // ray::Color Bar Hue control
RAYGUIAPI int GuiColorPickerHSV(ray::Rectangle bounds, const char *text, ray::Vector3 *colorHsv);                // ray::Color Picker control that avoids conversion to RGB on each call (multiple color controls)
RAYGUIAPI int GuiColorPanelHSV(ray::Rectangle bounds, const char *text, ray::Vector3 *colorHsv);                 // ray::Color Panel control that returns HSV color value, used by GuiColorPickerHSV()
//----------------------------------------------------------------------------------------------------------
...
#include <stdarg.h>             // Required for: va_list, va_start(), vfprintf(), va_end() [ray::TextFormat()]
...
static ray::Font guiFont = { 0 };                    // Gui current font (WARNING: highly coupled to raylib)
...
static ray::Rectangle guiSliderActive = { 0 };       // Gui slider active bounds ray::Rectangle, used as an unique identifier
...
#define ray::KEY_RIGHT           262
#define ray::KEY_LEFT            263
#define ray::KEY_DOWNY_UP            264
#define ray::KEY_UP              265
#define ray::KEY_BACKSPACE       259
#define ray::KEY_ENTER           257

#define ray::MOUSE_LEFT_BUTTON     0

// Input required functions
//-------------------------------------------------------------------------------
static ray::Vector2 ray::GetMousePosition(void);
static float ray::GetMouseWheelMove(void);
...
static int ray::GetCharPressed(void);         // -- GuiTextBox(), GuiValueBox()
//-------------------------------------------------------------------------------

// Drawing required functions
//-------------------------------------------------------------------------------
static void DrawRectangle(int x, int y, int width, int height, ray::Color color);        // -- GuiDrawRectangle()
static void DrawRectangleGradientEx(ray::Rectangle rec, ray::Color col1, ray::Color col2, ray::Color col3, ray::Color col4); // -- GuiColorPicker()
//-------------------------------------------------------------------------------

// Text required functions
//-------------------------------------------------------------------------------
static ray::Font ray::GetFontDefault(void);                            // -- GuiLoadStyleDefault()
static ray::Font ray::LoadFontEx(const char *fileName, int fontSize, int *codepoints, int codepointCount); // -- GuiLoadStyle(), load font

static Texture2D LoadTextureFromImage(ray::Image image);          // -- GuiLoadStyle(), required to load texture from embedded font atlas image
static void SetShapesTexture(Texture2D tex, ray::Rectangle rec);  // -- GuiLoadStyle(), required to set shapes rec to font white rec (optimization)

static char *ray::LoadFileText(const char *fileName);             // -- GuiLoadStyle(), required to load charset data
static void ray::UnloadFileText(char *text);                      // -- GuiLoadStyle(), required to unload charset data

static const char *ray::GetDirectoryPath(const char *filePath);   // -- GuiLoadStyle(), required to find charset/font file from text .rgs

static int *ray::LoadCodepoints(const char *text, int *count);    // -- GuiLoadStyle(), required to load required font codepoints list
static void ray::UnloadCodepoints(int *codepoints);               // -- GuiLoadStyle(), required to unload codepoints list

static unsigned char *ray::DecompressData(const unsigned char *compData, int compDataSize, int *dataSize); // -- GuiLoadStyle()
//-------------------------------------------------------------------------------

// raylib functions already implemented in raygui
//-------------------------------------------------------------------------------
static ray::Color ray::GetColor(int hexValue);                // Returns a ray::Color struct from hexadecimal value
static int ColorToInt(ray::Color color);                 // Returns hexadecimal value for a ray::Color
static bool CheckCollisionPointRec(ray::Vector2 point, ray::Rectangle rec);   // Check if point is inside ray::Rectangle
static const char *ray::TextFormat(const char *text, ...);               // Formatting of text with variables to 'embed'
static const char **TextSplit(const char *text, char delimiter, int *count);    // Split text into multiple strings
static int ray::TextToInteger(const char *text);         // Get integer value from text

static int ray::GetCodepointNext(const char *text, int *codepointSize);  // Get next codepoint in a UTF-8 encoded text
static const char *ray::CodepointToUTF8(int codepoint, int *byteSize);   // Encode codepoint into UTF-8 text (char array size returned as parameter)

static void DrawRectangleGradientV(int posX, int posY, int width, int height, ray::Color color1, ray::Color color2);  // Draw ray::Rectangle vertical gradient
//-------------------------------------------------------------------------------
...
static ray::Rectangle GetTextBounds(int control, ray::Rectangle bounds);  // Get text bounds considering control bounds
static const char *GetTextIcon(const char *text, int *iconId);  // Get text icon if provided and move text cursor

static void GuiDrawText(const char *text, ray::Rectangle textBounds, int alignment, ray::Color tint);     // Gui draw text using default font
static void GuiDrawRectangle(ray::Rectangle rec, int borderWidth, ray::Color borderColor, ray::Color color);   // Gui draw ray::Rectangle using default raygui style
...
static ray::Vector3 ConvertHSVtoRGB(ray::Vector3 hsv);                    // Convert color data from HSV to RGB
static ray::Vector3 ConvertRGBtoHSV(ray::Vector3 rgb);                    // Convert color data from RGB to HSV

static int GuiScrollBar(ray::Rectangle bounds, int value, int minValue, int maxValue);   // Scroll bar control, used by GuiScrollPanel()
static void GuiTooltip(ray::Rectangle controlRec);                   // Draw tooltip using control rec position

static ray::Color GuiFade(ray::Color color, float alpha);         // Fade color by an alpha factor
...
// Set custom gui font
// NOTE: ray::Font loading/unloading is external to raygui
void GuiSetFont(ray::Font font)
{
...
// Window Box control
int GuiWindowBox(ray::Rectangle bounds, const char *title)
{
    ...
    ray::Rectangle statusBar = { bounds.x, bounds.y, bounds.width, (float)statusBarHeight };
    if (bounds.height < statusBarHeight*2.0f) bounds.height = statusBarHeight*2.0f;

    ray::Rectangle windowPanel = { bounds.x, bounds.y + (float)statusBarHeight - 1, bounds.width, bounds.height - (float)statusBarHeight + 1 };
    ray::Rectangle closeButtonRec = { statusBar.x + statusBar.width - GuiGetStyle(STATUSBAR, BORDER_WIDTH) - 20,
...
int GuiGroupBox(ray::Rectangle bounds, const char *text)
{
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x, bounds.y + bounds.height - 1, bounds.width, RAYGUI_GROUPBOX_LINE_THICK }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));
    GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + bounds.width - 1, bounds.y, RAYGUI_GROUPBOX_LINE_THICK, bounds.height }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR)));

    GuiLine(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x, bounds.y - GuiGetStyle(DEFAULT, TEXT_SIZE)/2, bounds.width, (float)GuiGetStyle(DEFAULT, TEXT_SIZE) }, text);
    //--------------------------------------------------------------------
...
// Line control
int GuiLine(ray::Rectangle bounds, const char *text)
{
    ...
    ray::Color color = ray::GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED : LINE_COLOR));

    // Draw control
    //--------------------------------------------------------------------
    if (text == NULL) GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x, bounds.y + bounds.height/2, bounds.width, 1 }, 0, ray::BLANK, color);
    else
    {
        ray::Rectangle textBounds = { 0 };
        ...
        GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x, bounds.y + bounds.height/2, RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1 }, 0, ray::BLANK, color);
        GuiDrawText(text, textBounds, TEXT_ALIGN_LEFT, color);
        GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + 12 + textBounds.width + 4, bounds.y + bounds.height/2, bounds.width - textBounds.width - RAYGUI_LINE_MARGIN_TEXT - RAYGUI_LINE_TEXT_PADDING, 1 }, 0, ray::BLANK, color);
    }
    //--------------------------------------------------------------------
...
// Panel control
int GuiPanel(ray::Rectangle bounds, const char *text)
{
    ...
    ray::Rectangle statusBar = { bounds.x, bounds.y, bounds.width, (float)RAYGUI_WINDOWBOX_STATUSBAR_HEIGHT };
    ...
    GuiDrawRectangle(bounds, RAYGUI_PANEL_BORDER_WIDTH, ray::GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BORDER_COLOR_DISABLED: LINE_COLOR)),
                     ray::GetColor(GuiGetStyle(DEFAULT, (state == STATE_DISABLED)? BASE_COLOR_DISABLED : BACKGROUND_COLOR)));
    //--------------------------------------------------------------------
...
int GuiTabBar(ray::Rectangle bounds, const char **text, int count, int *active)
{
    ...
    ray::Rectangle tabBounds = { bounds.x, bounds.y, RAYGUI_TABBAR_ITEM_WIDTH, bounds.height };

    if (*active < 0) *active = 0;
    else if (*active > count - 1) *active = count - 1;

    int offsetX = 0;    // Required in case tabs go out of screen
    offsetX = (*active + 2)*RAYGUI_TABBAR_ITEM_WIDTH - ray::GetScreenWidth();
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < ray::GetScreenWidth())
        {
            ...
            GuiSetStyle(BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
#if defined(RAYGUI_NO_ICONS)
            if (GuiButton(RAYGUI_CLITERAL(ray::Rectangle){ tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14 }, "x")) result = i;
#else
            if (GuiButton(RAYGUI_CLITERAL(ray::Rectangle){ tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14 }, GuiIconText(ICON_CROSS_SMALL, NULL))) result = i;
#endif
            GuiSetStyle(BUTTON, BORDER_WIDTH, tempBorderWidth);
            GuiSetStyle(BUTTON, TEXT_ALIGNMENT, tempTextAlignment);
        }
    ...
    GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x, bounds.y + bounds.height - 1, bounds.width, 1 }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(TOGGLE, BORDER_COLOR_NORMAL)));
    //--------------------------------------------------------------------
...
int GuiScrollPanel(ray::Rectangle bounds, const char *text, ray::Rectangle content, ray::Vector2 *scroll, ray::Rectangle *view)
{
    ...
    ray::Rectangle temp = { 0 };
    if (view == NULL) view = &temp;

    ray::Vector2 scrollPos = { 0.0f, 0.0f };
    ...
    ray::Rectangle horizontalScrollBar = {
    ...
    ray::Rectangle verticalScrollBar = {
    ...
    *view = (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)?
                RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + verticalScrollBarWidth + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth, bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth } :
                RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth, bounds.height - 2*GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth };
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        {
            if (IsMouseButtonDown(ray::MOUSE_LEFT_BUTTON)) state = STATE_PRESSED;
            else state = STATE_FOCUSED;

#if defined(SUPPORT_SCROLLBAR_KEY_INPUT)
            if (hasHorizontalScrollBar)
            {
                if (IsKeyDown(ray::KEY_RIGHT)) scrollPos.x -= GuiGetStyle(SCROLLBAR, SCROLL_SPEED);
                if (IsKeyDown(ray::KEY_LEFT)) scrollPos.x += GuiGetStyle(SCROLLBAR, SCROLL_SPEED);
            }

            if (hasVerticalScrollBar)
            {
                if (IsKeyDown(ray::KEY_DOWNY_UP)) scrollPos.y -= GuiGetStyle(SCROLLBAR, SCROLL_SPEED);
                if (IsKeyDown(ray::KEY_UP)) scrollPos.y += GuiGetStyle(SCROLLBAR, SCROLL_SPEED);
            }
#endif
            float wheelMove = ray::GetMouseWheelMove();

            // Horizontal and vertical scrolling with mouse wheel
            if (hasHorizontalScrollBar && (IsKeyDown(ray::KEY_LEFT_CONTROL) || IsKeyDown(ray::KEY_LEFT_SHIFT))) scrollPos.x += wheelMove*mouseWheelSpeed;
    ...
    GuiDrawRectangle(bounds, 0, ray::BLANK, ray::GetColor(GuiGetStyle(DEFAULT, BACKGROUND_COLOR)));        // Draw background
    ...
    // Draw detail corner ray::Rectangle if both scroll bars are visible
    if (hasHorizontalScrollBar && hasVerticalScrollBar)
    {
        ray::Rectangle corner = { (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE)? (bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH) + 2) : (horizontalScrollBar.x + horizontalScrollBar.width + 2), verticalScrollBar.y + verticalScrollBar.height + 2, (float)horizontalScrollBarWidth - 4, (float)verticalScrollBarWidth - 4 };
        GuiDrawRectangle(corner, 0, ray::BLANK, ray::GetColor(GuiGetStyle(LISTVIEW, TEXT + (state*3))));
    }

    // Draw scrollbar lines depending on current state
    GuiDrawRectangle(bounds, GuiGetStyle(DEFAULT, BORDER_WIDTH), ray::GetColor(GuiGetStyle(LISTVIEW, BORDER + (state*3))), ray::BLANK);
...
// Label control
int GuiLabel(ray::Rectangle bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawText(text, GetTextBounds(LABEL, bounds), GuiGetStyle(LABEL, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(LABEL, TEXT + (state*3))));
    //--------------------------------------------------------------------
...
int GuiButton(ray::Rectangle bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            if (IsMouseButtonReleased(ray::MOUSE_LEFT_BUTTON)) result = 1;
        }
    }
    //--------------------------------------------------------------------
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(BUTTON, BORDER_WIDTH), ray::GetColor(GuiGetStyle(BUTTON, BORDER + (state*3))), ray::GetColor(GuiGetStyle(BUTTON, BASE + (state*3))));
    GuiDrawText(text, GetTextBounds(BUTTON, bounds), GuiGetStyle(BUTTON, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(BUTTON, TEXT + (state*3))));
...
int GuiLabelButton(ray::Rectangle bounds, const char *text)
{
    GuiState state = guiState;
    bool pressed = false;
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            if (IsMouseButtonReleased(ray::MOUSE_LEFT_BUTTON)) pressed = true;
        }
    }
    //--------------------------------------------------------------------
...
int GuiToggle(ray::Rectangle bounds, const char *text, bool *active)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        {
            if (IsMouseButtonDown(ray::MOUSE_LEFT_BUTTON)) state = STATE_PRESSED;
            else if (IsMouseButtonReleased(ray::MOUSE_LEFT_BUTTON))
            {
                state = STATE_NORMAL;
                *active = !(*active);
            }
    ...
    {
        GuiDrawRectangle(bounds, GuiGetStyle(TOGGLE, BORDER_WIDTH), ray::GetColor(GuiGetStyle(TOGGLE, ((*active)? BORDER_COLOR_PRESSED : (BORDER + state*3)))), ray::GetColor(GuiGetStyle(TOGGLE, ((*active)? BASE_COLOR_PRESSED : (BASE + state*3)))));
        GuiDrawText(text, GetTextBounds(TOGGLE, bounds), GuiGetStyle(TOGGLE, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(TOGGLE, ((*active)? TEXT_COLOR_PRESSED : (TEXT + state*3)))));
    }
    else
    {
        GuiDrawRectangle(bounds, GuiGetStyle(TOGGLE, BORDER_WIDTH), ray::GetColor(GuiGetStyle(TOGGLE, BORDER + state*3)), ray::GetColor(GuiGetStyle(TOGGLE, BASE + state*3)));
        GuiDrawText(text, GetTextBounds(TOGGLE, bounds), GuiGetStyle(TOGGLE, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(TOGGLE, TEXT + state*3)));
    }
...
int GuiToggleGroup(ray::Rectangle bounds, const char *text, int *active)
{
...
int GuiToggleSlider(ray::Rectangle bounds, const char *text, int *active)
{
    int result = 0;
    GuiState state = guiState;
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(SLIDER, BORDER_WIDTH), ray::GetColor(GuiGetStyle(TOGGLE, BORDER + (state*3))),
        ray::GetColor(GuiGetStyle(TOGGLE, BASE_COLOR_NORMAL)));

    // Draw internal slider
    if (state == STATE_NORMAL) GuiDrawRectangle(slider, 0, ray::BLANK, ray::GetColor(GuiGetStyle(SLIDER, BASE_COLOR_PRESSED)));
    else if (state == STATE_FOCUSED) GuiDrawRectangle(slider, 0, ray::BLANK, ray::GetColor(GuiGetStyle(SLIDER, BASE_COLOR_FOCUSED)));
    else if (state == STATE_PRESSED) GuiDrawRectangle(slider, 0, ray::BLANK, ray::GetColor(GuiGetStyle(SLIDER, BASE_COLOR_PRESSED)));
    ...
    if (text != NULL)
    {
        ...
        GuiDrawText(items[*active], textBounds, GuiGetStyle(TOGGLE, TEXT_ALIGNMENT), Fade(ray::GetColor(GuiGetStyle(TOGGLE, TEXT + (state*3))), guiAlpha));
    }
    //--------------------------------------------------------------------
...
int GuiCheckBox(ray::Rectangle bounds, const char *text, bool *checked)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, totalBounds))
        {
            ...
            if (IsMouseButtonReleased(ray::MOUSE_LEFT_BUTTON))
            {
                *checked = !(*checked);
                result = 1;
            }
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(CHECKBOX, BORDER_WIDTH), ray::GetColor(GuiGetStyle(CHECKBOX, BORDER + (state*3))), ray::BLANK);

    if (*checked)
    {
        ray::Rectangle check = { bounds.x + GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING),
                            bounds.y + GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING),
                            bounds.width - 2*(GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING)),
                            bounds.height - 2*(GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING)) };
        GuiDrawRectangle(check, 0, ray::BLANK, ray::GetColor(GuiGetStyle(CHECKBOX, TEXT + state*3)));
    }

    GuiDrawText(text, textBounds, (GuiGetStyle(CHECKBOX, TEXT_ALIGNMENT) == TEXT_ALIGN_RIGHT)? TEXT_ALIGN_LEFT : TEXT_ALIGN_RIGHT, ray::GetColor(GuiGetStyle(LABEL, TEXT + (state*3))));
    //--------------------------------------------------------------------
...
int GuiComboBox(ray::Rectangle bounds, const char *text, int *active)
{
    int result = 0;
    GuiState state = guiState;
    ...
    ray::Rectangle selector = { (float)bounds.x + bounds.width + GuiGetStyle(COMBOBOX, COMBO_BUTTON_SPACING),
    ...
    //--------------------------------------------------------------------
    // Draw combo box main
    GuiDrawRectangle(bounds, GuiGetStyle(COMBOBOX, BORDER_WIDTH), ray::GetColor(GuiGetStyle(COMBOBOX, BORDER + (state*3))), ray::GetColor(GuiGetStyle(COMBOBOX, BASE + (state*3))));
    GuiDrawText(items[*active], GetTextBounds(COMBOBOX, bounds), GuiGetStyle(COMBOBOX, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(COMBOBOX, TEXT + (state*3))));
    ...
    GuiButton(selector, ray::TextFormat("%i/%i", *active + 1, itemCount));
...
int GuiDropdownBox(ray::Rectangle bounds, const char *text, int *active, bool editMode)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if ((state != STATE_DISABLED) && (editMode || !guiLocked) && (itemCount > 1) && !guiSliderDragging)
    {
        ...
        if (editMode)
        {
            ...
            {
                if (IsMouseButtonPressed(ray::MOUSE_LEFT_BUTTON) || IsMouseButtonReleased(ray::MOUSE_LEFT_BUTTON)) result = 1;
            }

            // Check if already selected item has been pressed again
            if (CheckCollisionPointRec(mousePoint, bounds) && IsMouseButtonPressed(ray::MOUSE_LEFT_BUTTON)) result = 1;

            // Check focused and selected item
            for (int i = 0; i < itemCount; i++)
            {
                // Update item ray::Rectangle y position for next item
    ...
    GuiDrawRectangle(bounds, GuiGetStyle(DROPDOWNBOX, BORDER_WIDTH), ray::GetColor(GuiGetStyle(DROPDOWNBOX, BORDER + state*3)), ray::GetColor(GuiGetStyle(DROPDOWNBOX, BASE + state*3)));
    GuiDrawText(items[itemSelected], GetTextBounds(DROPDOWNBOX, bounds), GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + state*3)));
    ...
    if (editMode)
    {
        ...
        for (int i = 0; i < itemCount; i++)
        {
            ...
            if (i == itemSelected)
            {
                GuiDrawRectangle(itemBounds, GuiGetStyle(DROPDOWNBOX, BORDER_WIDTH), ray::GetColor(GuiGetStyle(DROPDOWNBOX, BORDER_COLOR_PRESSED)), ray::GetColor(GuiGetStyle(DROPDOWNBOX, BASE_COLOR_PRESSED)));
                GuiDrawText(items[i], GetTextBounds(DROPDOWNBOX, itemBounds), GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(DROPDOWNBOX, TEXT_COLOR_PRESSED)));
            }
            else if (i == itemFocused)
            {
                GuiDrawRectangle(itemBounds, GuiGetStyle(DROPDOWNBOX, BORDER_WIDTH), ray::GetColor(GuiGetStyle(DROPDOWNBOX, BORDER_COLOR_FOCUSED)), ray::GetColor(GuiGetStyle(DROPDOWNBOX, BASE_COLOR_FOCUSED)));
                GuiDrawText(items[i], GetTextBounds(DROPDOWNBOX, itemBounds), GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(DROPDOWNBOX, TEXT_COLOR_FOCUSED)));
            }
            else GuiDrawText(items[i], GetTextBounds(DROPDOWNBOX, itemBounds), GuiGetStyle(DROPDOWNBOX, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(DROPDOWNBOX, TEXT_COLOR_NORMAL)));
        }
    }

    // Draw arrows (using icon if available)
#if defined(RAYGUI_NO_ICONS)
    GuiDrawText("v", RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + bounds.width - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height/2 - 2, 10, 10 },
                TEXT_ALIGN_CENTER, ray::GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))));
#else
    GuiDrawText("#120#", RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + bounds.width - GuiGetStyle(DROPDOWNBOX, ARROW_PADDING), bounds.y + bounds.height/2 - 6, 10, 10 },
                TEXT_ALIGN_CENTER, ray::GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))));   // ICON_ARROW_DOWN_FILL
#endif
    //--------------------------------------------------------------------
...
int GuiTextBox(ray::Rectangle bounds, char *text, int bufferSize, bool editMode)
{
    ...
    ray::Rectangle textBounds = GetTextBounds(TEXTBOX, bounds);
    int textWidth = GetTextWidth(text) - GetTextWidth(text + textBoxCursorIndex);
    int textIndexOffset = 0;    // Text index offset to start drawing in the box

    // Cursor ray::Rectangle
    // NOTE: Position X value should be updated
    ray::Rectangle cursor = {
    ...
    // Mouse cursor ray::Rectangle
    // NOTE: Initialized outside of screen
    ray::Rectangle mouseCursor = cursor;
    mouseCursor.x = -1;
    mouseCursor.width = 1;

    // Auto-cursor movement logic
    // NOTE: Cursor moves automatically when key down after some time
    if (IsKeyDown(ray::KEY_LEFT) || IsKeyDown(ray::KEY_RIGHT) || IsKeyDown(ray::KEY_UP) || IsKeyDown(ray::KEY_DOWN) || IsKeyDown(ray::KEY_BACKSPACE) || IsKeyDown(ray::KEY_DELETE)) autoCursorCooldownCounter++;
    else
    {
    ...
    {
        ray::Vector2 mousePosition = ray::GetMousePosition();
        ...
        if (editMode)
        {
            ...
            while (textWidth >= textBounds.width)
            {
                ...
                ray::GetCodepointNext(text + textIndexOffset, &nextCodepointSize);
            ...
            int codepoint = ray::GetCharPressed();       // Get Unicode codepoint
            if (multiline && IsKeyPressed(ray::KEY_ENTER)) codepoint = (int)'\n';

            if (textBoxCursorIndex > textLength) textBoxCursorIndex = textLength;

            // Encode codepoint as UTF-8
            int codepointSize = 0;
            const char *charEncoded = ray::CodepointToUTF8(codepoint, &codepointSize);
            ...
            if ((textLength > 0) && IsKeyPressed(ray::KEY_HOME)) textBoxCursorIndex = 0;

            // Move cursor to end
            if ((textLength > textBoxCursorIndex) && IsKeyPressed(ray::KEY_END)) textBoxCursorIndex = textLength;

            // Delete codepoint from text, after current cursor position
            if ((textLength > textBoxCursorIndex) && (IsKeyPressed(ray::KEY_DELETE) || (IsKeyDown(ray::KEY_DELETE) && (autoCursorCooldownCounter >= RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN))))
            {
                autoCursorDelayCounter++;

                if (IsKeyPressed(ray::KEY_DELETE) || (autoCursorDelayCounter%RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames
                {
                    int nextCodepointSize = 0;
                    ray::GetCodepointNext(text + textBoxCursorIndex, &nextCodepointSize);
            ...
            if ((textLength > 0) && (IsKeyPressed(ray::KEY_BACKSPACE) || (IsKeyDown(ray::KEY_BACKSPACE) && (autoCursorCooldownCounter >= RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN))))
            {
                autoCursorDelayCounter++;

                if (IsKeyPressed(ray::KEY_BACKSPACE) || (autoCursorDelayCounter%RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames
                {
                    int prevCodepointSize = 0;
                    ray::GetCodepointPrevious(text + textBoxCursorIndex, &prevCodepointSize);
            ...
            if (IsKeyPressed(ray::KEY_LEFT) || (IsKeyDown(ray::KEY_LEFT) && (autoCursorCooldownCounter > RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN)))
            {
                autoCursorDelayCounter++;

                if (IsKeyPressed(ray::KEY_LEFT) || (autoCursorDelayCounter%RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames
                {
                    int prevCodepointSize = 0;
                    ray::GetCodepointPrevious(text + textBoxCursorIndex, &prevCodepointSize);
            ...
            else if (IsKeyPressed(ray::KEY_RIGHT) || (IsKeyDown(ray::KEY_RIGHT) && (autoCursorCooldownCounter > RAYGUI_TEXTBOX_AUTO_CURSOR_COOLDOWN)))
            {
                autoCursorDelayCounter++;

                if (IsKeyPressed(ray::KEY_RIGHT) || (autoCursorDelayCounter%RAYGUI_TEXTBOX_AUTO_CURSOR_DELAY) == 0)      // Delay every movement some frames
                {
                    int nextCodepointSize = 0;
                    ray::GetCodepointNext(text + textBoxCursorIndex, &nextCodepointSize);
            ...
            if (CheckCollisionPointRec(mousePosition, textBounds))     // Mouse hover text
            {
                ...
                {
                    codepoint = ray::GetCodepointNext(&text[i], &codepointSize);
                ...
                if (ray::GetMousePosition().x >= (textBounds.x + textEndWidth - glyphWidth/2))
                {
                ...
                if ((mouseCursor.x >= 0) && IsMouseButtonPressed(ray::MOUSE_LEFT_BUTTON))
                {
            ...
            if ((!multiline && IsKeyPressed(ray::KEY_ENTER)) ||
                (!CheckCollisionPointRec(mousePosition, bounds) && IsMouseButtonPressed(ray::MOUSE_LEFT_BUTTON)))
            {
    ...
    {
        GuiDrawRectangle(bounds, GuiGetStyle(TEXTBOX, BORDER_WIDTH), ray::GetColor(GuiGetStyle(TEXTBOX, BORDER + (state*3))), ray::GetColor(GuiGetStyle(TEXTBOX, BASE_COLOR_PRESSED)));
    }
    else if (state == STATE_DISABLED)
    {
        GuiDrawRectangle(bounds, GuiGetStyle(TEXTBOX, BORDER_WIDTH), ray::GetColor(GuiGetStyle(TEXTBOX, BORDER + (state*3))), ray::GetColor(GuiGetStyle(TEXTBOX, BASE_COLOR_DISABLED)));
    }
    else GuiDrawRectangle(bounds, GuiGetStyle(TEXTBOX, BORDER_WIDTH), ray::GetColor(GuiGetStyle(TEXTBOX, BORDER + (state*3))), ray::BLANK);

    // Draw text considering index offset if required
    // NOTE: Text index offset depends on cursor position
    GuiDrawText(text + textIndexOffset, textBounds, GuiGetStyle(TEXTBOX, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(TEXTBOX, TEXT + (state*3))));

    // Draw cursor
    if (editMode && !GuiGetStyle(TEXTBOX, TEXT_READONLY))
    {
        //if (autoCursorMode || ((blinkCursorFrameCounter/40)%2 == 0))
        GuiDrawRectangle(cursor, 0, ray::BLANK, ray::GetColor(GuiGetStyle(TEXTBOX, BORDER_COLOR_PRESSED)));

        // Draw mouse position cursor (if required)
        if (mouseCursor.x >= 0) GuiDrawRectangle(mouseCursor, 0, ray::BLANK, ray::GetColor(GuiGetStyle(TEXTBOX, BORDER_COLOR_PRESSED)));
    }
...
bool GuiTextBoxMulti(ray::Rectangle bounds, char *text, int bufferSize, bool editMode)
{
    bool pressed = false;
...
int GuiSpinner(ray::Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    int result = 1;
    GuiState state = guiState;
    ...
    ray::Rectangle spinner = { bounds.x + GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH) + GuiGetStyle(SPINNER, SPIN_BUTTON_SPACING), bounds.y,
                          bounds.width - 2*(GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH) + GuiGetStyle(SPINNER, SPIN_BUTTON_SPACING)), bounds.height };
    ray::Rectangle leftButtonBound = { (float)bounds.x, (float)bounds.y, (float)GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.height };
    ray::Rectangle rightButtonBound = { (float)bounds.x + bounds.width - GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.y, (float)GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.height };
    ...
    GuiDrawText(text, textBounds, (GuiGetStyle(SPINNER, TEXT_ALIGNMENT) == TEXT_ALIGN_RIGHT)? TEXT_ALIGN_LEFT : TEXT_ALIGN_RIGHT, ray::GetColor(GuiGetStyle(LABEL, TEXT + (state*3))));
    //--------------------------------------------------------------------
...
int GuiValueBox(ray::Rectangle bounds, const char *text, int *value, int minValue, int maxValue, bool editMode)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (editMode)
        {
            ...
            if (keyCount > 0)
            {
                if (IsKeyPressed(ray::KEY_BACKSPACE))
                {
                    keyCount--;
            ...
            if (valueHasChanged) *value = ray::TextToInteger(textValue);

            // NOTE: We are not clamp values until user input finishes
            //if (*value > maxValue) *value = maxValue;
            //else if (*value < minValue) *value = minValue;

            if (IsKeyPressed(ray::KEY_ENTER) || (!CheckCollisionPointRec(mousePoint, bounds) && IsMouseButtonPressed(ray::MOUSE_LEFT_BUTTON))) result = 1;
        }
        else
        {
            if (*value > maxValue) *value = maxValue;
            ...
            {
                state = STATE_FOCUSED;
                if (IsMouseButtonPressed(ray::MOUSE_LEFT_BUTTON)) result = 1;
            }
        }
    }
    //--------------------------------------------------------------------
    ...
    // Draw control
    //--------------------------------------------------------------------
    ray::Color baseColor = ray::BLANK;
    if (state == STATE_PRESSED) baseColor = ray::GetColor(GuiGetStyle(VALUEBOX, BASE_COLOR_PRESSED));
    else if (state == STATE_DISABLED) baseColor = ray::GetColor(GuiGetStyle(VALUEBOX, BASE_COLOR_DISABLED));

    GuiDrawRectangle(bounds, GuiGetStyle(VALUEBOX, BORDER_WIDTH), ray::GetColor(GuiGetStyle(VALUEBOX, BORDER + (state*3))), baseColor);
    GuiDrawText(textValue, GetTextBounds(VALUEBOX, bounds), TEXT_ALIGN_CENTER, ray::GetColor(GuiGetStyle(VALUEBOX, TEXT + (state*3))));

    // Draw cursor
    if (editMode)
    {
        // NOTE: ValueBox internal text is always centered
        ray::Rectangle cursor = { bounds.x + GetTextWidth(textValue)/2 + bounds.width/2 + 1, bounds.y + 2*GuiGetStyle(VALUEBOX, BORDER_WIDTH), 4, bounds.height - 4*GuiGetStyle(VALUEBOX, BORDER_WIDTH) };
        GuiDrawRectangle(cursor, 0, ray::BLANK, ray::GetColor(GuiGetStyle(VALUEBOX, BORDER_COLOR_PRESSED)));
    }

    // Draw text label if provided
    GuiDrawText(text, textBounds, (GuiGetStyle(VALUEBOX, TEXT_ALIGNMENT) == TEXT_ALIGN_RIGHT)? TEXT_ALIGN_LEFT : TEXT_ALIGN_RIGHT, ray::GetColor(GuiGetStyle(LABEL, TEXT + (state*3))));
    //--------------------------------------------------------------------
...
int GuiSliderPro(ray::Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue, int sliderWidth)
{
    int result = 0;
    ...
    ray::Rectangle slider = { bounds.x, bounds.y + GuiGetStyle(SLIDER, BORDER_WIDTH) + GuiGetStyle(SLIDER, SLIDER_PADDING),
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        {
            if (IsMouseButtonDown(ray::MOUSE_LEFT_BUTTON))
            {
            ...
            else
            {
                ...
                guiSliderActive = RAYGUI_CLITERAL(ray::Rectangle){ 0, 0, 0, 0 };
            }
        }
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(SLIDER, BORDER_WIDTH), ray::GetColor(GuiGetStyle(SLIDER, BORDER + (state*3))), ray::GetColor(GuiGetStyle(SLIDER, (state != STATE_DISABLED)?  BASE_COLOR_NORMAL : BASE_COLOR_DISABLED)));

    // Draw slider internal bar (depends on state)
    if (state == STATE_NORMAL) GuiDrawRectangle(slider, 0, ray::BLANK, ray::GetColor(GuiGetStyle(SLIDER, BASE_COLOR_PRESSED)));
    else if (state == STATE_FOCUSED) GuiDrawRectangle(slider, 0, ray::BLANK, ray::GetColor(GuiGetStyle(SLIDER, TEXT_COLOR_FOCUSED)));
    else if (state == STATE_PRESSED) GuiDrawRectangle(slider, 0, ray::BLANK, ray::GetColor(GuiGetStyle(SLIDER, TEXT_COLOR_PRESSED)));
    ...
    if (textLeft != NULL)
    {
        ...
        GuiDrawText(textLeft, textBounds, TEXT_ALIGN_RIGHT, ray::GetColor(GuiGetStyle(SLIDER, TEXT + (state*3))));
    }
    ...
    if (textRight != NULL)
    {
        ...
        GuiDrawText(textRight, textBounds, TEXT_ALIGN_LEFT, ray::GetColor(GuiGetStyle(SLIDER, TEXT + (state*3))));
    }
    //--------------------------------------------------------------------
...
int GuiSlider(ray::Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, GuiGetStyle(SLIDER, SLIDER_WIDTH));
}

// Slider Bar control extended, returns selected value
int GuiSliderBar(ray::Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, 0);
}

// Progress Bar control extended, shows current progress value
int GuiProgressBar(ray::Rectangle bounds, const char *textLeft, const char *textRight, float *value, float minValue, float maxValue)
{
    int result = 0;
    GuiState state = guiState;
    ...
    // Progress bar
    ray::Rectangle progress = { bounds.x + GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
    ...
    {
        GuiDrawRectangle(bounds, GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), ray::GetColor(GuiGetStyle(PROGRESSBAR, BORDER + (state*3))), ray::BLANK);
    }
    else
    {
        if (*value > minValue)
        {
            ...
            GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x, bounds.y, (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
            GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x, bounds.y + 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height - 2 }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
            GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x, bounds.y + bounds.height - 1, (int)progress.width + (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
        }
        else GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x, bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));

        if (*value >= maxValue) GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + progress.width + 1, bounds.y, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_FOCUSED)));
        else
        {
            // Draw borders not yet reached by value
            GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + (int)progress.width + 1, bounds.y, bounds.width - (int)progress.width - 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + (int)progress.width + 1, bounds.y + bounds.height - 1, bounds.width - (int)progress.width - 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH) }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
            GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + bounds.width - 1, bounds.y + 1, (float)GuiGetStyle(PROGRESSBAR, BORDER_WIDTH), bounds.height - 2 }, 0, ray::BLANK, ray::GetColor(GuiGetStyle(PROGRESSBAR, BORDER_COLOR_NORMAL)));
        }

        // Draw slider internal progress bar (depends on state)
        GuiDrawRectangle(progress, 0, ray::BLANK, ray::GetColor(GuiGetStyle(PROGRESSBAR, BASE_COLOR_PRESSED)));
    }
    ...
    if (textLeft != NULL)
    {
        ...
        GuiDrawText(textLeft, textBounds, TEXT_ALIGN_RIGHT, ray::GetColor(GuiGetStyle(PROGRESSBAR, TEXT + (state*3))));
    }
    ...
    if (textRight != NULL)
    {
        ...
        GuiDrawText(textRight, textBounds, TEXT_ALIGN_LEFT, ray::GetColor(GuiGetStyle(PROGRESSBAR, TEXT + (state*3))));
    }
    //--------------------------------------------------------------------
...
// Status Bar control
int GuiStatusBar(ray::Rectangle bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(STATUSBAR, BORDER_WIDTH), ray::GetColor(GuiGetStyle(STATUSBAR, BORDER + (state*3))), ray::GetColor(GuiGetStyle(STATUSBAR, BASE + (state*3))));
    GuiDrawText(text, GetTextBounds(STATUSBAR, bounds), GuiGetStyle(STATUSBAR, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(STATUSBAR, TEXT + (state*3))));
    //--------------------------------------------------------------------

    return result;
}

// Dummy ray::Rectangle control, intended for placeholding
int GuiDummyRec(ray::Rectangle bounds, const char *text)
{
    int result = 0;
    GuiState state = guiState;
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, 0, ray::BLANK, ray::GetColor(GuiGetStyle(DEFAULT, (state != STATE_DISABLED)? BASE_COLOR_NORMAL : BASE_COLOR_DISABLED)));
    GuiDrawText(text, GetTextBounds(DEFAULT, bounds), TEXT_ALIGN_CENTER, ray::GetColor(GuiGetStyle(BUTTON, (state != STATE_DISABLED)? TEXT_COLOR_NORMAL : TEXT_COLOR_DISABLED)));
    //------------------------------------------------------------------
...
// List View control
int GuiListView(ray::Rectangle bounds, const char *text, int *scrollIndex, int *active)
{
    int result = 0;
    int itemCount = 0;
...
int GuiListViewEx(ray::Rectangle bounds, const char **text, int count, int *scrollIndex, int *active, int *focus)
{
    int result = 0;
    GuiState state = guiState;
    ...
    // Define base item ray::Rectangle [0]
    ray::Rectangle itemBounds = { 0 };
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            if (useScrollBar)
            {
                int wheelMove = (int)ray::GetMouseWheelMove();
        ...
        // Reset item ray::Rectangle y to [0]
        itemBounds.y = bounds.y + GuiGetStyle(LISTVIEW, LIST_ITEMS_SPACING) + GuiGetStyle(DEFAULT, BORDER_WIDTH);
    }
    //--------------------------------------------------------------------
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(DEFAULT, BORDER_WIDTH), ray::GetColor(GuiGetStyle(LISTVIEW, BORDER + state*3)), ray::GetColor(GuiGetStyle(DEFAULT, BACKGROUND_COLOR)));     // Draw background
    ...
    for (int i = 0; ((i < visibleItems) && (text != NULL)); i++)
    {
        if (state == STATE_DISABLED)
        {
            if ((startIndex + i) == itemSelected) GuiDrawRectangle(itemBounds, GuiGetStyle(LISTVIEW, BORDER_WIDTH), ray::GetColor(GuiGetStyle(LISTVIEW, BORDER_COLOR_DISABLED)), ray::GetColor(GuiGetStyle(LISTVIEW, BASE_COLOR_DISABLED)));

            GuiDrawText(text[startIndex + i], GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_DISABLED)));
        }
        else
        {
            ...
            {
                // Draw item selected
                GuiDrawRectangle(itemBounds, GuiGetStyle(LISTVIEW, BORDER_WIDTH), ray::GetColor(GuiGetStyle(LISTVIEW, BORDER_COLOR_PRESSED)), ray::GetColor(GuiGetStyle(LISTVIEW, BASE_COLOR_PRESSED)));
                GuiDrawText(text[startIndex + i], GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_PRESSED)));
            }
            else if (((startIndex + i) == itemFocused)) // && (focus != NULL))  // NOTE: We want items focused, despite not returned!
            {
                // Draw item focused
                GuiDrawRectangle(itemBounds, GuiGetStyle(LISTVIEW, BORDER_WIDTH), ray::GetColor(GuiGetStyle(LISTVIEW, BORDER_COLOR_FOCUSED)), ray::GetColor(GuiGetStyle(LISTVIEW, BASE_COLOR_FOCUSED)));
                GuiDrawText(text[startIndex + i], GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_FOCUSED)));
            }
            else
            {
                // Draw item normal
                GuiDrawText(text[startIndex + i], GetTextBounds(DEFAULT, itemBounds), GuiGetStyle(LISTVIEW, TEXT_ALIGNMENT), ray::GetColor(GuiGetStyle(LISTVIEW, TEXT_COLOR_NORMAL)));
            }
        }
...
// ray::Color Panel control
int GuiColorPanel(ray::Rectangle bounds, const char *text, ray::Color *color)
{
    int result = 0;
    GuiState state = guiState;
    ray::Vector2 pickerSelector = { 0 };

    const ray::Color colWhite = { 255, 255, 255, 255 };
    const ray::Color colBlack = { 0, 0, 0, 255 };

    ray::Vector3 vcolor = { (float)color->r/255.0f, (float)color->g/255.0f, (float)color->b/255.0f };
    ray::Vector3 hsv = ConvertRGBtoHSV(vcolor);

    pickerSelector.x = bounds.x + (float)hsv.y*bounds.width;            // HSV: Saturation
    pickerSelector.y = bounds.y + (1.0f - (float)hsv.z)*bounds.height;  // HSV: Value

    ray::Vector3 maxHue = { hsv.x, 1.0f, 1.0f };
    ray::Vector3 rgbHue = ConvertHSVtoRGB(maxHue);
    ray::Color maxHueCol = { (unsigned char)(255.0f*rgbHue.x),
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            if (IsMouseButtonDown(ray::MOUSE_LEFT_BUTTON))
            {
                ...
                ray::Vector2 colorPick = { pickerSelector.x - bounds.x, pickerSelector.y - bounds.y };
                ...
                ray::Vector3 rgb = ConvertHSVtoRGB(hsv);

                // NOTE: Vector3ToColor() only available on raylib 1.8.1
                *color = RAYGUI_CLITERAL(ray::Color){ (unsigned char)(255.0f*rgb.x),
    ...
    if (state != STATE_DISABLED)
    {
        ...
        ray::Rectangle selector = { pickerSelector.x - GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)/2, pickerSelector.y - GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE)/2, (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE), (float)GuiGetStyle(COLORPICKER, COLOR_SELECTOR_SIZE) };
        GuiDrawRectangle(selector, 0, ray::BLANK, colWhite);
    }
    else
    {
        DrawRectangleGradientEx(bounds, Fade(Fade(ray::GetColor(GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)), 0.1f), guiAlpha), Fade(Fade(colBlack, 0.6f), guiAlpha), Fade(Fade(colBlack, 0.6f), guiAlpha), Fade(Fade(ray::GetColor(GuiGetStyle(COLORPICKER, BORDER_COLOR_DISABLED)), 0.6f), guiAlpha));
    }

    GuiDrawRectangle(bounds, GuiGetStyle(COLORPICKER, BORDER_WIDTH), ray::GetColor(GuiGetStyle(COLORPICKER, BORDER + state*3)), ray::BLANK);
    //--------------------------------------------------------------------

    return result;
}

// ray::Color Bar Alpha control
// NOTE: Returns alpha value normalized [0..1]
int GuiColorBarAlpha(ray::Rectangle bounds, const char *text, float *alpha)
{
    ...
    int result = 0;
    GuiState state = guiState;
    ray::Rectangle selector = { (float)bounds.x + (*alpha)*bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT)/2, (float)bounds.y - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT), (float)bounds.height + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2 };
    ...
    if (state != STATE_DISABLED)
    {
        ...
        for (int x = 0; x < checksX; x++)
        {
            ...
            {
                ray::Rectangle check = { bounds.x + x*RAYGUI_COLORBARALPHA_CHECKED_SIZE, bounds.y + y*RAYGUI_COLORBARALPHA_CHECKED_SIZE, RAYGUI_COLORBARALPHA_CHECKED_SIZE, RAYGUI_COLORBARALPHA_CHECKED_SIZE };
                GuiDrawRectangle(check, 0, ray::BLANK, ((x + y)%2)? Fade(ray::GetColor(GuiGetStyle(COLORPICKER, BORDER_COLOR_DISABLED)), 0.4f) : Fade(ray::GetColor(GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)), 0.4f));
            }
        }

        DrawRectangleGradientEx(bounds, RAYGUI_CLITERAL(ray::Color){ 255, 255, 255, 0 }, RAYGUI_CLITERAL(ray::Color){ 255, 255, 255, 0 }, Fade(RAYGUI_CLITERAL(ray::Color){ 0, 0, 0, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(ray::Color){ 0, 0, 0, 255 }, guiAlpha));
    }
    else DrawRectangleGradientEx(bounds, Fade(ray::GetColor(GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)), 0.1f), Fade(ray::GetColor(GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)), 0.1f), Fade(ray::GetColor(GuiGetStyle(COLORPICKER, BORDER_COLOR_DISABLED)), guiAlpha), Fade(ray::GetColor(GuiGetStyle(COLORPICKER, BORDER_COLOR_DISABLED)), guiAlpha));

    GuiDrawRectangle(bounds, GuiGetStyle(COLORPICKER, BORDER_WIDTH), ray::GetColor(GuiGetStyle(COLORPICKER, BORDER + state*3)), ray::BLANK);

    // Draw alpha bar: selector
    GuiDrawRectangle(selector, 0, ray::BLANK, ray::GetColor(GuiGetStyle(COLORPICKER, BORDER + state*3)));
    //--------------------------------------------------------------------

    return result;
}

// ray::Color Bar Hue control
// Returns hue value normalized [0..1]
// NOTE: Other similar bars (for reference):
//      ray::Color GuiColorBarSat() [WHITE->color]
//      ray::Color GuiColorBarValue() [BLACK->color], HSV/HSL
//      float GuiColorBarLuminance() [BLACK->WHITE]
int GuiColorBarHue(ray::Rectangle bounds, const char *text, float *hue)
{
    int result = 0;
    GuiState state = guiState;
    ray::Rectangle selector = { (float)bounds.x - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)bounds.y + (*hue)/360.0f*bounds.height - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT)/2, (float)bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW)*2, (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT) };
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        else if (CheckCollisionPointRec(mousePoint, bounds) || CheckCollisionPointRec(mousePoint, selector))
        {
            ...
            /*if (IsKeyDown(ray::KEY_UP))
            {
                hue -= 2.0f;
                if (hue <= 0.0f) hue = 0.0f;
            }
            else if (IsKeyDown(ray::KEY_DOWNY_UP))
            {
                hue += 2.0f;
    ...
    if (state != STATE_DISABLED)
    {
        ...
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y), (int)bounds.width, (int)ceilf(bounds.height/6), Fade(RAYGUI_CLITERAL(ray::Color){ 255, 0, 0, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(ray::Color){ 255, 255, 0, 255 }, guiAlpha));
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + bounds.height/6), (int)bounds.width, (int)ceilf(bounds.height/6), Fade(RAYGUI_CLITERAL(ray::Color){ 255, 255, 0, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(ray::Color){ 0, 255, 0, 255 }, guiAlpha));
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 2*(bounds.height/6)), (int)bounds.width, (int)ceilf(bounds.height/6), Fade(RAYGUI_CLITERAL(ray::Color){ 0, 255, 0, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(ray::Color){ 0, 255, 255, 255 }, guiAlpha));
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 3*(bounds.height/6)), (int)bounds.width, (int)ceilf(bounds.height/6), Fade(RAYGUI_CLITERAL(ray::Color){ 0, 255, 255, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(ray::Color){ 0, 0, 255, 255 }, guiAlpha));
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 4*(bounds.height/6)), (int)bounds.width, (int)ceilf(bounds.height/6), Fade(RAYGUI_CLITERAL(ray::Color){ 0, 0, 255, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(ray::Color){ 255, 0, 255, 255 }, guiAlpha));
        DrawRectangleGradientV((int)bounds.x, (int)(bounds.y + 5*(bounds.height/6)), (int)bounds.width, (int)(bounds.height/6), Fade(RAYGUI_CLITERAL(ray::Color){ 255, 0, 255, 255 }, guiAlpha), Fade(RAYGUI_CLITERAL(ray::Color){ 255, 0, 0, 255 }, guiAlpha));
    }
    else DrawRectangleGradientV((int)bounds.x, (int)bounds.y, (int)bounds.width, (int)bounds.height, Fade(Fade(ray::GetColor(GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)), 0.1f), guiAlpha), Fade(ray::GetColor(GuiGetStyle(COLORPICKER, BORDER_COLOR_DISABLED)), guiAlpha));
...
// ray::Color Picker control
// NOTE: It's divided in multiple controls:
//      ray::Color GuiColorPanel(ray::Rectangle bounds, ray::Color color)
//      float GuiColorBarAlpha(ray::Rectangle bounds, float alpha)
//      float GuiColorBarHue(ray::Rectangle bounds, float value)
// NOTE: bounds define GuiColorPanel() size
int GuiColorPicker(ray::Rectangle bounds, const char *text, ray::Color *color)
{
    int result = 0;

    ray::Color temp = { 200, 0, 0, 255 };
    if (color == NULL) color = &temp;

    GuiColorPanel(bounds, NULL, color);

    ray::Rectangle boundsHue = { (float)bounds.x + bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_PADDING), (float)bounds.y, (float)GuiGetStyle(COLORPICKER, HUEBAR_WIDTH), (float)bounds.height };
    //ray::Rectangle boundsAlpha = { bounds.x, bounds.y + bounds.height + GuiGetStyle(COLORPICKER, BARS_PADDING), bounds.width, GuiGetStyle(COLORPICKER, BARS_THICK) };

    ray::Vector3 hsv = ConvertRGBtoHSV(RAYGUI_CLITERAL(ray::Vector3){ (*color).r/255.0f, (*color).g/255.0f, (*color).b/255.0f });
    ...
    *color = RAYGUI_CLITERAL(ray::Color){ (unsigned char)roundf(rgb.x*255.0f), (unsigned char)roundf(rgb.y*255.0f), (unsigned char)roundf(rgb.z*255.0f), (*color).a };

    return result;
}

// ray::Color Picker control that avoids conversion to RGB and back to HSV on each call, thus avoiding jittering.
// The user can call ConvertHSVtoRGB() to convert *colorHsv value to RGB.
// NOTE: It's divided in multiple controls:
//      int GuiColorPanelHSV(ray::Rectangle bounds, const char *text, ray::Vector3 *colorHsv)
//      int GuiColorBarAlpha(ray::Rectangle bounds, const char *text, float *alpha)
//      float GuiColorBarHue(ray::Rectangle bounds, float value)
// NOTE: bounds define GuiColorPanelHSV() size
int GuiColorPickerHSV(ray::Rectangle bounds, const char *text, ray::Vector3 *colorHsv)
{
    int result = 0;

    ray::Vector3 tempHsv = { 0 };

    if (colorHsv == NULL)
    {
        const ray::Vector3 tempColor = { 200.0f/255.0f, 0.0f, 0.0f };
    ...
    const ray::Rectangle boundsHue = { (float)bounds.x + bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_PADDING), (float)bounds.y, (float)GuiGetStyle(COLORPICKER, HUEBAR_WIDTH), (float)bounds.height };
...
// ray::Color Panel control, returns HSV color value in *colorHsv.
...
int GuiColorPanelHSV(ray::Rectangle bounds, const char *text, ray::Vector3 *colorHsv)
{
    ...
    ray::Vector3 maxHue = { colorHsv->x, 1.0f, 1.0f };
    ray::Vector3 rgbHue = ConvertHSVtoRGB(maxHue);
    ray::Color maxHueCol = { (unsigned char)(255.0f*rgbHue.x),
                      (unsigned char)(255.0f*rgbHue.y),
                      (unsigned char)(255.0f*rgbHue.z), 255 };
...
// Message Box control
int GuiMessageBox(ray::Rectangle bounds, const char *title, const char *message, const char *buttons)
{
...
int GuiTextInputBox(ray::Rectangle bounds, const char *title, const char *message, const char *buttons, char *text, int textMaxSize, bool *secretViewActive)
{
    ...
    {
        static char stars[] = "****************";
        if (GuiTextBox(RAYGUI_CLITERAL(ray::Rectangle){ textBoxBounds.x, textBoxBounds.y, textBoxBounds.width - 4 - RAYGUI_TEXTINPUTBOX_HEIGHT, textBoxBounds.height },
            ((*secretViewActive == 1) || textEditMode)? text : stars, textMaxSize, textEditMode)) textEditMode = !textEditMode;

        GuiToggle(RAYGUI_CLITERAL(ray::Rectangle){ textBoxBounds.x + textBoxBounds.width - RAYGUI_TEXTINPUTBOX_HEIGHT, textBoxBounds.y, RAYGUI_TEXTINPUTBOX_HEIGHT, RAYGUI_TEXTINPUTBOX_HEIGHT }, (*secretViewActive == 1)? "#44#" : "#45#", secretViewActive);
    }
    else
    {
...
int GuiGrid(ray::Rectangle bounds, const char *text, float spacing, int subdivs, ray::Vector2 *mouseCell)
{
    ...
    ray::Vector2 mousePoint = ray::GetMousePosition();
    ray::Vector2 currentMouseCell = { -1, -1 };
    ...
    if (subdivs > 0)
    {
        ...
        {
            ray::Rectangle lineV = { bounds.x + spacing*i/subdivs, bounds.y, 1, bounds.height };
            GuiDrawRectangle(lineV, 0, ray::BLANK, ((i%subdivs) == 0)? GuiFade(ray::GetColor(color), RAYGUI_GRID_ALPHA*4) : GuiFade(ray::GetColor(color), RAYGUI_GRID_ALPHA));
        }

        // Draw horizontal grid lines
        for (int i = 0; i < linesH; i++)
        {
            ray::Rectangle lineH = { bounds.x, bounds.y + spacing*i/subdivs, bounds.width, 1 };
            GuiDrawRectangle(lineH, 0, ray::BLANK, ((i%subdivs) == 0)? GuiFade(ray::GetColor(color), RAYGUI_GRID_ALPHA*4) : GuiFade(ray::GetColor(color), RAYGUI_GRID_ALPHA));
        }
    }
                ...
                switch (buffer[0])
                {
                    ...
                    case 'f':
                    {
                        ...
                        if (charmapFileName[0] != '0')
                        {
                            ...
                            char *textData = ray::LoadFileText(ray::TextFormat("%s/%s", ray::GetDirectoryPath(fileName), charmapFileName));
                            codepoints = ray::LoadCodepoints(textData, &codepointCount);
                            ray::UnloadFileText(textData);
                        }

                        if (fontFileName[0] != '\0')
                        {
                            // In case a font is already loaded and it is not default internal font, unload it
                            if (font.texture.id != ray::GetFontDefault().texture.id) UnloadTexture(font.texture);

                            if (codepointCount > 0) font = ray::LoadFontEx(ray::TextFormat("%s/%s", ray::GetDirectoryPath(fileName), fontFileName), fontSize, codepoints, codepointCount);
                            else font = ray::LoadFontEx(ray::TextFormat("%s/%s", ray::GetDirectoryPath(fileName), fontFileName), fontSize, NULL, 0);   // Default to 95 standard codepoints
                        }
...
void GuiLoadStyleDefault(void)
{
    ...
    if (guiFont.texture.id != ray::GetFontDefault().texture.id)
    {
        ...
        guiFont = ray::GetFontDefault();

        // NOTE: Default raylib font character 95 is a white square
        ray::Rectangle whiteChar = guiFont.recs[95];

        // NOTE: We set up a 1px padding on char ray::Rectangle to avoid pixel bleeding on MSAA filtering
        SetShapesTexture(guiFont.texture, RAYGUI_CLITERAL(ray::Rectangle){ whiteChar.x + 1, whiteChar.y + 1, whiteChar.width - 2, whiteChar.height - 2 });
    }
}
...
void GuiDrawIcon(int iconId, int posX, int posY, int pixelSize, ray::Color color)
{
    ...
    for (int i = 0, y = 0; i < RAYGUI_ICON_SIZE*RAYGUI_ICON_SIZE/32; i++)
    {
        for (int k = 0; k < 32; k++)
        {
            ...
            {
            #if !defined(RAYGUI_STANDALONE)
                GuiDrawRectangle(RAYGUI_CLITERAL(ray::Rectangle){ (float)posX + (k%RAYGUI_ICON_SIZE)*pixelSize, (float)posY + y*pixelSize, (float)pixelSize, (float)pixelSize }, 0, ray::BLANK, color);
            #endif
            }
...
static void GuiLoadStyleFromMemory(const unsigned char *fileData, int dataSize)
{
        ...
        (signature[3] == ' '))
    {
        ...
        // ray::Font loading is highly dependant on raylib API to load font data and image
        ...
        if (fontDataSize > 0)
        {
            ...
            // Load font white ray::Rectangle
            ray::Rectangle fontWhiteRec = { 0 };
            memcpy(&fontWhiteRec, fileDataPtr, sizeof(ray::Rectangle));
            fileDataPtr += 16;
            ...
            {
                // Compressed font atlas image data (DEFLATE), it requires ray::DecompressData()
                ...
                imFont.data = ray::DecompressData(compData, fontImageCompSize, &dataUncompSize);
            ...
            {
                // ray::Font atlas image data is not compressed
            ...
            {
                // Load font recs data
                int recsDataSize = font.glyphCount*sizeof(ray::Rectangle);
                ...
                if ((recsDataCompressedSize > 0) && (recsDataCompressedSize != recsDataSize))
                {
                    ...
                    font.recs = (ray::Rectangle *)ray::DecompressData(recsDataCompressed, recsDataCompressedSize, &recsDataUncompSize);
                ...
                else
                {
                    ...
                    font.recs = (ray::Rectangle *)RAYGUI_CALLOC(font.glyphCount, sizeof(ray::Rectangle));
                    for (int i = 0; i < font.glyphCount; i++)
                    {
                        memcpy(&font.recs[i], fileDataPtr, sizeof(ray::Rectangle));
                        fileDataPtr += sizeof(ray::Rectangle);
                    }
                }
                ...
                font.glyphs = (ray::GlyphInfo *)RAYGUI_CALLOC(font.glyphCount, sizeof(ray::GlyphInfo));
                ...
                if ((glyphsDataCompressedSize > 0) && (glyphsDataCompressedSize != glyphsDataSize))
                {
                    ...
                    unsigned char *glyphsDataUncomp = ray::DecompressData(glypsDataCompressed, glyphsDataCompressedSize, &glyphsDataUncompSize);
            ...
            else font = ray::GetFontDefault();   // Fallback in case of errors loading font atlas texture

            GuiSetFont(font);

            // Set font texture source ray::Rectangle to be used as white texture to draw shapes
...
static int GetTextWidth(const char *text)
{
    ...
    if ((text != NULL) && (text[0] != '\0'))
    {
        ...
        if ((guiFont.texture.id > 0) && (text != NULL))
        {
            ...
            {
                int codepoint = ray::GetCodepointNext(&text[i], &codepointSize);
...
static ray::Rectangle GetTextBounds(int control, ray::Rectangle bounds)
{
    ray::Rectangle textBounds = bounds;
...
static const char *GetTextIcon(const char *text, int *iconId)
{
    ...
    *iconId = -1;
    if (text[0] == '#')     // Maybe we have an icon!
    {
        ...
        if (text[pos] == '#')
        {
            *iconId = ray::TextToInteger(iconValue);
...
static float GetNextSpaceWidth(const char *text, int *nextSpaceIndex)
{
    ...
    for (int i = 0; text[i] != '\0'; i++)
    {
        if (text[i] != ' ')
        {
            codepoint = ray::GetCodepoint(&text[i], &codepointByteCount);
...
static void GuiDrawText(const char *text, ray::Rectangle textBounds, int alignment, ray::Color tint)
{
    ...
    for (int i = 0; i < lineCount; i++)
    {
        ...
        //---------------------------------------------------------------------------------
        ray::Vector2 textBoundsPosition = { textBounds.x, textBounds.y };
        ...
        {
            int codepoint = ray::GetCodepointNext(&lines[i][c], &codepointSize);
            ...
            else
            {
                ...
                if ((codepoint != ' ') && (codepoint != '\t'))      // Do not draw codepoints with no glyph
                {
                    if (wrapMode == TEXT_WRAP_NONE)
                    {
                        ...
                        {
                            DrawTextCodepoint(guiFont, codepoint, RAYGUI_CLITERAL(ray::Vector2){ textBoundsPosition.x + textOffsetX, textBoundsPosition.y + textOffsetY }, (float)GuiGetStyle(DEFAULT, TEXT_SIZE), GuiFade(tint, guiAlpha));
                        }
                    }
...
// Gui draw ray::Rectangle using default raygui plain style with borders
static void GuiDrawRectangle(ray::Rectangle rec, int borderWidth, ray::Color borderColor, ray::Color color)
{
    if (color.a > 0)
    {
        // Draw ray::Rectangle filled with color
        DrawRectangle((int)rec.x, (int)rec.y, (int)rec.width, (int)rec.height, GuiFade(color, guiAlpha));
    }

    if (borderWidth > 0)
    {
        // Draw ray::Rectangle border lines with color
...
static void GuiTooltip(ray::Rectangle controlRec)
{
    if (!guiLocked && guiTooltip && (guiTooltipPtr != NULL) && !guiSliderDragging)
    {
        ray::Vector2 textSize = MeasureTextEx(GuiGetFont(), guiTooltipPtr, (float)GuiGetStyle(DEFAULT, TEXT_SIZE), (float)GuiGetStyle(DEFAULT, TEXT_SPACING));

        if ((controlRec.x + textSize.x + 16) > ray::GetScreenWidth()) controlRec.x -= (textSize.x + 16 - controlRec.width);

        GuiPanel(RAYGUI_CLITERAL(ray::Rectangle){ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.f }, NULL);

        int textPadding = GuiGetStyle(LABEL, TEXT_PADDING);
        int textAlignment = GuiGetStyle(LABEL, TEXT_ALIGNMENT);
        GuiSetStyle(LABEL, TEXT_PADDING, 0);
        GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
        GuiLabel(RAYGUI_CLITERAL(ray::Rectangle){ controlRec.x, controlRec.y + controlRec.height + 4, textSize.x + 16, GuiGetStyle(DEFAULT, TEXT_SIZE) + 8.f }, guiTooltipPtr);
...
// NOTE: ray::Color data should be passed normalized
static ray::Vector3 ConvertRGBtoHSV(ray::Vector3 rgb)
{
    ray::Vector3 hsv = { 0 };
    float min = 0.0f;
    float max = 0.0f;
...
// NOTE: ray::Color data should be passed normalized
static ray::Vector3 ConvertHSVtoRGB(ray::Vector3 hsv)
{
    ray::Vector3 rgb = { 0 };
...
static int GuiScrollBar(ray::Rectangle bounds, int value, int minValue, int maxValue)
{
    GuiState state = guiState;
    ...
    // Arrow buttons [<] [>] [∧] [∨]
    ray::Rectangle arrowUpLeft = { 0 };
    ray::Rectangle arrowDownRight = { 0 };

    // Actual area of the scrollbar excluding the arrow buttons
    ray::Rectangle scrollbar = { 0 };

    // Slider bar that moves     --[///]-----
    ray::Rectangle slider = { 0 };
    ...
    arrowUpLeft = RAYGUI_CLITERAL(ray::Rectangle){
    ...
    if (isVertical)
    {
        arrowDownRight = RAYGUI_CLITERAL(ray::Rectangle){ (float)bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)bounds.y + bounds.height - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize };
        scrollbar = RAYGUI_CLITERAL(ray::Rectangle){ bounds.x + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING), arrowUpLeft.y + arrowUpLeft.height, bounds.width - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING)), bounds.height - arrowUpLeft.height - arrowDownRight.height - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH) };
        ...
        slider = RAYGUI_CLITERAL(ray::Rectangle){
    ...
    {
        arrowDownRight = RAYGUI_CLITERAL(ray::Rectangle){ (float)bounds.x + bounds.width - spinnerSize - GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH), (float)spinnerSize, (float)spinnerSize };
        scrollbar = RAYGUI_CLITERAL(ray::Rectangle){ arrowUpLeft.x + arrowUpLeft.width, bounds.y + GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING), bounds.width - arrowUpLeft.width - arrowDownRight.width - 2*GuiGetStyle(SCROLLBAR, BORDER_WIDTH), bounds.height - 2*(GuiGetStyle(SCROLLBAR, BORDER_WIDTH) + GuiGetStyle(SCROLLBAR, SCROLL_PADDING)) };
    ...
    if ((state != STATE_DISABLED) && !guiLocked)
    {
        ...
        else if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            // Handle mouse wheel
            int wheel = (int)ray::GetMouseWheelMove();
            ...
            if (isVertical)
            {
                if (IsKeyDown(ray::KEY_DOWNY_UP)) value += 5;
                else if (IsKeyDown(ray::KEY_UP)) value -= 5;
            }
            else
            {
                if (IsKeyDown(ray::KEY_RIGHT)) value += 5;
                else if (IsKeyDown(ray::KEY_LEFT)) value -= 5;
            }
            */
        }
    ...
    // Draw control
    //--------------------------------------------------------------------
    GuiDrawRectangle(bounds, GuiGetStyle(SCROLLBAR, BORDER_WIDTH), ray::GetColor(GuiGetStyle(LISTVIEW, BORDER + state*3)), ray::GetColor(GuiGetStyle(DEFAULT, BORDER_COLOR_DISABLED)));   // Draw the background

    GuiDrawRectangle(scrollbar, 0, ray::BLANK, ray::GetColor(GuiGetStyle(BUTTON, BASE_COLOR_NORMAL)));     // Draw the scrollbar active area background
    GuiDrawRectangle(slider, 0, ray::BLANK, ray::GetColor(GuiGetStyle(SLIDER, BORDER + state*3)));         // Draw the slider bar
    ...
    if (GuiGetStyle(SCROLLBAR, ARROWS_VISIBLE))
    {
        ...
        GuiDrawText(isVertical? "^" : "<",
            RAYGUI_CLITERAL(ray::Rectangle){ arrowUpLeft.x, arrowUpLeft.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
            TEXT_ALIGN_CENTER, ray::GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))));
        GuiDrawText(isVertical? "v" : ">",
            RAYGUI_CLITERAL(ray::Rectangle){ arrowDownRight.x, arrowDownRight.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
            TEXT_ALIGN_CENTER, ray::GetColor(GuiGetStyle(DROPDOWNBOX, TEXT + (state*3))));
#else
        GuiDrawText(isVertical? "#121#" : "#118#",
            RAYGUI_CLITERAL(ray::Rectangle){ arrowUpLeft.x, arrowUpLeft.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
            TEXT_ALIGN_CENTER, ray::GetColor(GuiGetStyle(SCROLLBAR, TEXT + state*3)));   // ICON_ARROW_UP_FILL / ICON_ARROW_LEFT_FILL
        GuiDrawText(isVertical? "#120#" : "#119#",
            RAYGUI_CLITERAL(ray::Rectangle){ arrowDownRight.x, arrowDownRight.y, isVertical? bounds.width : bounds.height, isVertical? bounds.width : bounds.height },
            TEXT_ALIGN_CENTER, ray::GetColor(GuiGetStyle(SCROLLBAR, TEXT + state*3)));   // ICON_ARROW_DOWN_FILL / ICON_ARROW_RIGHT_FILL
#endif
    }
    //--------------------------------------------------------------------
...
// ray::Color fade-in or fade-out, alpha goes from 0.0f to 1.0f
// WARNING: It multiplies current alpha by alpha scale factor
static ray::Color GuiFade(ray::Color color, float alpha)
{
    if (alpha < 0.0f) alpha = 0.0f;
    else if (alpha > 1.0f) alpha = 1.0f;

    ray::Color result = { color.r, color.g, color.b, (unsigned char)(color.a*alpha) };

    return result;
}

#if defined(RAYGUI_STANDALONE)
// Returns a ray::Color struct from hexadecimal value
static ray::Color ray::GetColor(int hexValue)
{
    ray::Color color;
...
// Returns hexadecimal value for a ray::Color
static int ColorToInt(ray::Color color)
{
    return (((int)color.r << 24) | ((int)color.g << 16) | ((int)color.b << 8) | (int)color.a);
}

// Check if point is inside ray::Rectangle
static bool CheckCollisionPointRec(ray::Vector2 point, ray::Rectangle rec)
{
...
static const char *ray::TextFormat(const char *text, ...)
{
...
// Draw ray::Rectangle with vertical gradient fill color
// NOTE: This function is only used by GuiColorPicker()
static void DrawRectangleGradientV(int posX, int posY, int width, int height, ray::Color color1, ray::Color color2)
{
    ray::Rectangle bounds = { (float)posX, (float)posY, (float)width, (float)height };
...
static int ray::TextToInteger(const char *text)
{
    int value = 0;
    int sign = 1;
...
static const char *ray::CodepointToUTF8(int codepoint, int *byteSize)
{
...
static int ray::GetCodepointNext(const char *text, int *codepointSize)
{
    const char *ptr = text;
